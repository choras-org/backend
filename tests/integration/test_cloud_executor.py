...
def mock_sftp_on_ssh(mock_ssh, mock_sftp):
    """Attach mock_sftp to mock_ssh.open_sftp() as a context manager."""
    ...
    # also support direct (non-context-manager) usage
    mock_ssh.open_sftp.return_value = mock_sftp


# =============================================================================
# U1–U5: _parse_overall_progress
...
class TestParseOverallProgress:
    ...
    def test_single_result_at_100(self):
        ...
        """
        json_data = {"results": [{"percentage": 100}]}
        assert CloudExecutor._parse_overall_progress(json_data) == 100
    ...
    def test_results_entry_missing_percentage_key(self):
        ...
        """
        json_data = {"results": [{"no_percentage": 1}]}
        result = CloudExecutor._parse_overall_progress(json_data)
...
# =============================================================================
# I7–I9: _build_singularity_image
...
# =============================================================================
# I10–I11: _execute_singularity_image
...
# =============================================================================
# I12–I16: _poll_until_complete — happy path
# Partitions: EP-P1, EP-P2, EP-P3, EP-P6, EP-P7
# =============================================================================
...
# =============================================================================
# I17–I19: _collect_outputs_and_cleanup
...
# =============================================================================
# I20: execute() — happy path
# Partitions: EP-S1, EP-M1, EP-M2, EP-M3, EP-G1
# =============================================================================
...
class TestExecuteHappyPath:
    ...
    def test_execute_returns_completed_job(self, tmp_path):
        ...
        sim_config = {"env": {"JSON_PATH": json_path}}

        executor = make_executor()
        patches = self._all_ssh_patches()

        with patch.multiple(CloudExecutor, **patches):
            result = executor.execute(self._method_config(), sim_config)
    ...
    def test_execute_calls_poll_until_complete(self, tmp_path):
        ...
        with patch.multiple(CloudExecutor, **patches):
            executor.execute(self._method_config(), {"env": {"JSON_PATH": json_path}})

        patches["_poll_until_complete"].assert_called_once()

    def test_execute_uploads_tar_and_input_files(self, tmp_path):
        """
        I22 — EP-S1
        execute() uploads tar, JSON, msh, and geo to remote host.
        """
        json_path = str(tmp_path / "input.json")
        with open(json_path, "w") as f:
            json.dump({
...
# =============================================================================
# B9–B16: execute() — bad day (via TestCloudExecutorExecute)
# Partitions: EP-S2, EP-S4, EP-S5, EP-S6, EP-P4, EP-P5, EP-P6
# =============================================================================

class TestExecuteBadDay:
    ...
    def test_ssh_authentication_fails(self, tmp_path):
        """
        B9 — EP-S2
        _mkdir_remote raises SSHCommandError (auth failure) →
        SSHCommandError propagates out of execute().
        Sandbox left behind unclean.
        """
        patches = self._patch_all_ssh()
        patches["_mkdir_remote"] = MagicMock(
            side_effect=SSHCommandError("SSH authentication failed")
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
            with pytest.raises(SSHCommandError, match="SSH authentication failed"):
                executor.execute(self._method_config(), self._sim_config(tmp_path))

    def test_sftp_upload_tar_fails_halfway(self, tmp_path):
        """
        B10 — EP-S4
        _upload_file_via_sftp raises mid-transfer →
        exception propagates before build starts.
        """
        patches = self._patch_all_ssh()
        patches["_upload_file_via_sftp"] = MagicMock(
            side_effect=Exception("SFTP upload interrupted")
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
            with pytest.raises(Exception, match="SFTP upload interrupted"):
    ...
    def test_build_singularity_image_fails(self, tmp_path):
        """
        B11 — EP-S5
        _build_singularity_image raises (disk full / corrupt tar) →
        exception propagates, Singularity never launched, no cleanup runs.
        """
        patches = self._patch_all_ssh()
        patches["_build_singularity_image"] = MagicMock(
            side_effect=SSHCommandError("Disk full on remote")
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
            with pytest.raises(SSHCommandError, match="Disk full on remote"):
                executor.execute(self._method_config(), self._sim_config(tmp_path))

    def test_remote_json_never_reaches_100(self, tmp_path):
        """
        B12 — EP-P5
        _poll_until_complete raises RuntimeError (stall timeout) →
        exception propagates out of execute().
        KNOWN BUG: no timeout exists yet — documents expected behaviour
        once fix is applied.
        """
        patches = self._patch_all_ssh()
        patches["_poll_until_complete"] = MagicMock(
            side_effect=RuntimeError(
                "Aborting — job exceeded timeout. Last known progress: 50%"
            )
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
    ...
    def test_remote_json_always_corrupt(self, tmp_path):
        """
        B13 — EP-P4
        _poll_until_complete raises RuntimeError (max failed cycles) →
        exception propagates.
        KNOWN BUG: no max failed cycle count exists yet.
        """
        patches = self._patch_all_ssh()
        patches["_poll_until_complete"] = MagicMock(
            side_effect=RuntimeError(
                "Aborting — JSON unreadable for 10 consecutive cycles"
            )
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
            with pytest.raises(RuntimeError, match="JSON unreadable"):
                executor.execute(self._method_config(), self._sim_config(tmp_path))

    def test_cancel_flag_created_before_polling(self, tmp_path):
        """
        B14 — EP-P6
        Cancel flag exists before polling starts →
        _poll_until_complete returns immediately, execute() returns _CompletedJob.
        """
        patches = self._patch_all_ssh()
        patches["_poll_until_complete"] = MagicMock(return_value=None)
    ...
    def test_collect_outputs_and_cleanup_fails_mid_download(self, tmp_path):
        """
        B15 — EP-S4
        Network drop during output download →
        _poll_until_complete propagates the error out of execute().
        Partial files on disk; sandbox not cleaned up.
        """
        patches = self._patch_all_ssh()
        patches["_poll_until_complete"] = MagicMock(
            side_effect=Exception("Network error during output download")
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
            with pytest.raises(Exception, match="Network error during output download"):
                executor.execute(self._method_config(), self._sim_config(tmp_path))

    def test_remote_sandbox_already_exists(self, tmp_path):
        """
        B16 — EP-S6
        Sandbox already exists from a previous failed run with same task_id →
        SSHCommandError raised from _build_singularity_image.
        """
        patches = self._patch_all_ssh()
        patches["_build_singularity_image"] = MagicMock(
            side_effect=SSHCommandError(
                "Sandbox dg_image_sif_task-001 already exists on remote"
            )
        )
        executor = self._executor()
        with patch.multiple(CloudExecutor, **patches):
            with pytest.raises(SSHCommandError, match="already exists on remote"):
                executor.execute(self._method_config(), self._sim_config(tmp_path))


# =============================================================================
# I24–I25: cancel()
# Partitions: EP-S1, EP-S2
# =============================================================================
...
class TestCancel:
    ...
    def test_cancel_kills_processes_and_cleans_up(self):
        """
        I24 — EP-S1
        cancel() kills the Singularity container processes and removes
        the sandbox and tar from the remote host.
        """
        executor = make_executor()
        cancelation_info = {
            "container_image": "dg_image:latest",
    ...
    def test_cancel_constructs_correct_sandbox_name(self):
        ...
        the correct sandbox name used to identify processes.
        """
        executor = make_executor()
        cancelation_info = {
            "container_image": "dg_image:latest",
        ...
        assert called_with == "dg_image_sif_abc-123"
