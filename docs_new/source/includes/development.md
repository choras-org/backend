...
## Implementing your simulation method
...
Inside your new folder, create a file named `<your_method_name>Interface.py` (see `MyNewMethod_method/MyNewMethodInterface.py` for reference).
...
- A main interface function, e.g. `<your_method_name>_method()`, which:
  - Accepts the path to a `.json` input file as its argument. These files are automatically created by the CHORAS backend when a new simulation is started.
  - Interprets the `.json` file data for your method. This follows the general structure below (also see `simulation-backend/common/exampleInput_DE.json` for another example):
```json
{
...
```
  - Writes progress percentage and results into the same `.json` file. See above or `simulation-backend/common/exampleInput_DE.json` for where the results and progress percentage should be written.
- A `__main__` function that calls the above interface function for running the simulation, given a particular input file.

When simulations are created, `<your_method_name>Interface.py` is containerized, and the `__main__` function is executed, with the path to the `.json` file being passed to the container via an environment variable. If you wish to test `<your_method_name>Interface.py` by itself outside of a container, separate testing files should be created.

We strongly recommend implementing your simulation method by implementing the abstract `SimulationMethod` interface, for standardization purposes. Although this is not necessary (the `__main__` method can contain anything), it promotes best practices. See `MyNewMethod_method/MyNewMethodInterface.py` for an example of how to do this.

The `exampleInput_<method_name>.json` files inside `simulation-backend/common` are to be used for testing or reference purposes, they are not used by CHORAS in any way.
...
- Include all files your method needs (interface, shared geometry/helpers)
- Define `CMD` command to run the main interface file.

## Integrating your new method into CHORAS

#### 1. Add example settings
...
#### 2. Modify `simulation-backend/methods-config.json`

Update the root file `method-config.json` in the `simulation-backend` directory with a new entry.
...
#### 3. Modify Docker Image configuration
...
#### 4. Modify the application building script
