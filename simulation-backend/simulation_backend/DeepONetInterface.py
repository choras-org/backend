...
def _convert_relative_to_absolute_paths(json_file_path: str | Path, dirname: str) -> dict:
    """
    ...
    data["deeponet_train_setup"]["output_dir"] = _resolve_path(
        data["deeponet_train_setup"]["output_dir"],
        dirname
    )
...
def _load_and_process_dg_results(output_path: str, output_filename: str, file_format: str) -> tuple:
    """
...
def _parse_receiver_position_from_filename(filename: str) -> tuple[list[float], list[float]]:
    """
    ...
    source_pos = [float(x0_match.group(1)), float(x0_match.group(2)), float(x0_match.group(3))]
    receiver_pos = [float(r0_match.group(1)), float(r0_match.group(2)), float(r0_match.group(3))]
...
def _write_results_json(
    ...
    receivers_dir = os.path.join(
        settings["deeponet_train_setup"]["output_dir"],
        model_id,
        "figs",
        "receivers"
    )
...
def deeponet_method(json_file_path: str | Path, output_json_path: str | Path = None):
    ...
    if output_json_path is None:
        output_json_path = os.path.join(dirname, "headless_backend", "output", "results.json")

    # Step 2: Run DG simulation
    _run_dg_simulation(json_file_path)
    ...
    mesh, pressures, time_steps, source_positions, results_dg = _load_and_process_dg_results(
        output_path, output_filename, file_format
    )
...
if __name__ == "__main__":
    ...
    # Define output path
    results_json_path = os.path.join(dirname, "headless_backend", "output", "results.json")
    ...
    # Plot the results
    plot_results(results_json_path)